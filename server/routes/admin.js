...
router.post('/tournaments', checkAdmin, async (req, res) => {
  try {
    const { title, title_kz, title_ru, title_en, game, category, date, time, rules, rules_kz, rules_ru, rules_en, whatsappLink, maxPlayers } = req.body;
    
    // Handle both formats (object or separate fields)
    const titleKz = title_kz || title?.kz || title;
    const titleRu = title_ru || title?.ru || title;
    const titleEn = title_en || title?.en || title;
    const rulesKz = rules_kz || rules?.kz || rules;
    const rulesRu = rules_ru || rules?.ru || rules;
    const rulesEn = rules_en || rules?.en || rules;
    
    // Validate required fields
    if (!titleKz || !titleRu) {
      return res.status(400).json({ error: 'Title in Kazakh and Russian is required' });
    }
    ...
    if (!rulesKz || !rulesRu) {
      return res.status(400).json({ error: 'Rules in Kazakh and Russian are required' });
    }
...
router.put('/tournaments/:id', checkAdmin, async (req, res) => {
  try {
    const { title, title_kz, title_ru, title_en, game, category, date, time, rules, rules_kz, rules_ru, rules_en, whatsappLink, requireScreenRecord, status, maxPlayers } = req.body;
    
    const updateData = {};
    
    if (title_kz || title?.kz) updateData.title_kz = title_kz || title.kz;
    if (title_ru || title?.ru) updateData.title_ru = title_ru || title.ru;
    if (title_en || title?.en) updateData.title_en = title_en || title.en;
    ...
    if (rules_en || rules?.en) updateData.rules_en = rules_en || rules.en;
